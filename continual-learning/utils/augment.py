...
def random_indices(y, device="cuda"):
    n = len(y)
...
def cutmix(model, input, target, criterion, mix_p=0.5, beta=1.0):
    r = np.random.rand(1)
    if r < mix_p:
        # generate mixed sample
        lam = np.random.beta(beta, beta)
        rand_index = random_indices(target)
...
class DiffAug:
    def __init__(
        self,
        strategy="color_crop_flip_scale_rotate",
        batch=False,
        ratio_cutout=0.5,
        single=False,
    ):
